...
export function ProgramKerjaBendaharaClient({ profile }: { profile: Profile }) {
  ...
  const [unggulanForm, setUnggulanForm] = useState<FormState>(emptyForm);
  ...
  useEffect(() => {
    (async () => {
      const { data } = await supabase
        ...
        .order("nomor_divisi");
      setDivisiList((data ?? []) as DivisiOption[]);
      setLoading(false);
    })();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const selectedDivisi = divisiList.find((d) => d.id === divisiId) ?? null;
  ...
  async function load() {
    ...
    const { data: noteData } = await supabase
      ...
      .eq("id", divisiId)
      .maybeSingle();
    setNotesList(parseNoteList(noteData?.catatan_program_belum_terlaksana ?? ""));
  }
  ...
  async function uploadFile(file: File): Promise<{ name: string; path: string } | null> {
    ...
    const path = `${divisiId}/${Date.now()}-${file.name.replace(/[^a-zA-Z0-9.\-_]/g, "_")}`;
  ...
  async function handleSaveUnggulan() {
    if (!divisiId || !validateUnggulan()) return;
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader title="Pilih Divisi" subtitle="Pilih divisi yang ingin dikelola program kerjanya" />
        <CardContent>
      ...
      {!divisiId ? (
        <EmptyState
          ...
          description="Pilih divisi di atas untuk mulai mengelola program kerja divisi tersebut."
        />
      ) : (
        <>
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-lg font-semibold text-slate-900 dark:text-white">
                {selectedDivisi?.nama_divisi ?? "Divisi"}
              </h2>
            ...
            <CardContent>
              {unggulanItems.length === 0 ? (
                <EmptyState
                  ...
                  description="Unggah file program unggulan divisi ini."
                />
              ) : (
          ...
          <Card>
            ...
            <CardContent>
              {items.length === 0 ? (
                <EmptyState title="Belum ada program kerja" description="Tambahkan program kerja divisi ini." />
              ) : (
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  {items.map((p) => {
